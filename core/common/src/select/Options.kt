...
// The code below was copied from Okio:
// https://github.com/square/okio/blob/master/okio/src/commonMain/kotlin/okio/Options.kt
...
public class Options private constructor(
    ...
    override fun get(index: Int): ByteString = byteStrings[index]
    ...
    public companion object {
        ...
        private fun buildTrieRecursive(
            ...
            require(fromIndexIn < toIndex)
            for (i in fromIndexIn until toIndex) {
                require(byteStrings[i].size >= byteStringOffset)
            }

            var fromIndex = fromIndexIn
            ...
            if (from[byteStringOffset] != to[byteStringOffset]) {
                ...
                for (i in fromIndex until toIndex) {
                    ...
                    if (i == fromIndex || rangeByte != byteStrings[i - 1][byteStringOffset]) {
                        node.writeInt(rangeByte.toInt() and 0xff)
                    }
                }
                ...
                for (i in byteStringOffset until byteStringOffset + scanByteCount) {
                    node.writeInt(from[i].toInt() and 0xff)
                }
