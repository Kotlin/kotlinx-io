...
class BufferSourceJVMTest : AbstractBufferedSourceJVMTest(BufferedSourceFactory.BUFFER)
abstract class AbstractBufferedSourceJVMTest internal constructor(
  ...
  @Test fun bufferedPeek() {
    val buffer = Buffer()
    val peek = buffer.peekBuffered()

    buffer.writeByte(1).writeByte(2)
    assertEquals(1, peek.readByte())
    buffer.writeByte(3).writeByte(4)
    assertEquals(2, peek.readByte())
    assertEquals(3, peek.readByte())
    assertEquals(4, peek.readByte())
  }
...
//  }
  ...
  @Test fun select() {
    ...
    sink.emit()
    assertEquals(2, source.selectUsingBufferedPeekSource(options).toLong())
    assertEquals(','.code.toLong(), source.readByte().toLong())
    assertEquals(1, source.selectUsingBufferedPeekSource(options).toLong())
    assertEquals(','.code.toLong(), source.readByte().toLong())
    assertEquals(0, source.selectUsingBufferedPeekSource(options).toLong())
    assertTrue(source.exhausted())
  }
