...
actual interface ByteReadChannel {
    ...
    /**
     * Invokes [block] when it will be possible to read at least [consumer] bytes
     * providing byte buffer to it so lambda can read from the buffer
     * up to [ByteBuffer.remaining] bytes. If there are no [consumer] bytes available then the invocation could
     * suspend until the requirement will be met.
     *
     * If [consumer] is zero then the invocation will suspend until at least one byte available.
     *
     ...
     * in this case you have to invoke read again (with decreased [consumer] accordingly).
     *
     * It will fail with [EOFException] if not enough bytes ([availableForRead] < [consumer]) available
     ...
     *
     * @param consumer amount of bytes available for read, should be positive or zero
     * @param block to be invoked when at least [consumer] bytes available for read
     */
    suspend fun read(consumer: Int = 1, block: (ByteBuffer) -> Unit)
